...
public class Plane : MonoBehaviour
{
   ...
   public int rotateSpeed;
    ...
    void Landing() {
        ...
        Quaternion rotation = Quaternion.LookRotation(new Vector3(toCarrier.x, 0, toCarrier.z));
        transform.rotation = Quaternion.Lerp(transform.rotation, rotation, angularVelocity * Time.fixedDeltaTime);
    ...
    void FixedUpdate()
    {
        ...
        if (!timeToLand) {
            ...
            {
                dir = carrierTransform.position - transform.position;
                Quaternion rotation = Quaternion.LookRotation(new Vector3(dir.x, 0, dir.z));
                transform.rotation = Quaternion.Lerp(transform.rotation, rotation, angularVelocity * Time.fixedDeltaTime);
            } 
            
            //влет в круг
            else if ((patrol) && (dist <= flyRadius - dR))
            {
                dir = transform.position - carrierTransform.position;
                Quaternion rotation = Quaternion.LookRotation(new Vector3(dir.x, 0, dir.z));
                transform.rotation = Quaternion.Lerp(transform.rotation, rotation, angularVelocity * Time.fixedDeltaTime);
            }
